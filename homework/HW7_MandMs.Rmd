---
title: "HW7_MandMs"
---

In this assignment, you'll be picking up where you left off with the M&Ms activity. You'll load the data and perform some basic statistical analysis. Follow the steps below to complete the assignment.

### Task 1: Load the tidyverse

```{r}
library(tidyverse)
```


### Task 2: Load the M&Ms dataset from https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/mandms.csv

```{r}
mandms_data <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/mandms.csv")
```


### Task 3: Sample 5 rows of data from the dataset, and then sample 10 rows, saving each to separate dataframes called sample5 and sample10.

```{r}
sample_5 <- mandms_data |> 
  sample_n(5)
sample_10 <- mandms_data |> 
  sample_n(10)
```


### Task 4: Calculate the mean and standard deviation of the samples, using one of the colors.

```{r}
sample_5 |>  summarize(mean = mean(black), sd = sd(black))
```

```{r}
sample_10 |> summarize(mean = mean(black), sd=sd(black))
```


### Task 5: Calculate the mean and standard deviation of the population, using the same color as Task 4

```{r}
mandms_data |> summarize(mean = mean(black), sd = sd(black))
```


### Task 6: Compare the sample and population statistics

Compare the three sets of means and standard deviations. Are they similar?

The sample of five datapoints has a mean quite close to the population, while the sample of 10 has a mean considerably higher than the population. Conversely, the sample of five has a standard deviation much lower than the population, while the sample of 10 has a standard deviation more similar to the population.

### Task 7: Describe the results

Write a few sentences about what you found in the data. What does this tell you about the M&Ms dataset and about the act of random sampling?

This data tells me indicates that the M&Ms dataset, and the random scooping method that created it, is prone to outliers. The sample of 5 oddly did not have any true outliers in the number of black M&Ms, but the sample of 10 did. Statistical outliers are also evidenced in the relatively high standard deviation of the population, and the fact that the mean is 73.28 while the number of black M&Ms range from 40 to 116. This data exemplifies how random sampling can lead to samples that aren't representative of the population, seen in the disparity in mean and standard deviation between the samples and population.

When finished, save your work, switch to GitHub Desktop, then add, commit and push your changes to GitHub and submit the URL of the notebook in ELMS.
