...
```{r}
gdp_djia_data <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
gdp_djia_data |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
The Dow Jones Industrial Average and the gross domestic product of the United States have an extremely strong positive correlation. Their correlation coefficient is 0.95, nearing a perfect positive correlation with a coefficient at 1, and the line of best fit displayed on the scatterplot slopes smoothly from bottom left to top right. As the GDP increases, so does the DJIA.
