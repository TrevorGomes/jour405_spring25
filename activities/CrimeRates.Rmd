...
```{r}
pg_crime <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/md_annual_crime.csv")
```
...
```{r}
total_pg_crime <- pg_crime %>% mutate(crime_rate = (`GRAND TOTAL`/`POPULATION`)*100000)
```

### Task 5: Calculate the violent and property crime rates for each year (same unit)

```{r}
violent_crime <- total_pg_crime %>% mutate(violent_crime_rate = (`VIOLENT CRIME TOTAL`/`POPULATION`)*100000)
```

```{r}
property_crime <- violent_crime %>% mutate(property_crime_rate = (`PROPERTY CRIME TOTAL`/`POPULATION`)*100000)
```
