...
summer_crime_averages <- pg_crime_averages |> mutate(july_change = (`Jul` - `average_incidents`)/`average_incidents` * 100)
summer_crime_averages <- pg_crime_averages |> mutate(august_change = (`Aug` - `average_incidents`)/`average_incidents` * 100)
```
...
During the summer months, more severe crimes, such as sex offeneses, assault and homicide, are generally more common. Though the general theory that crime is more likely to occur in the summer months is not applicable to all crimes, the average number of crimes does increase during the summer for 11 categories, while only
...
Nutgraf: On average, accidents, stolen vehicles, theft from vehicles and accidents including vehicles being impounded each occur over 300 times per month in Maryland's second-most populous county. 
