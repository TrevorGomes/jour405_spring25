...
```{r}
sample_100 |> summarize(mean = mean(population), sd = sd(population))

sample_50 |> summarize(mean = mean(population), sd = sd(population))

sample_10 |> summarize(mean = mean(population), sd = sd(population))
```
...
```{r}
population |> summarize(mean = mean(population), sd = sd(population))
```
...
The mean and standard deviation of the sample and population are quite similar, falling within a few million (on the scale of populations, fairly close). With samples of 10 and 50, the results change quite a bit because the samples are more liable to being skewed. With country populations especially, outliers are fairly common.
